(() => {
  ...
  async function deleteProMeal(id) {
    data.meals=data.meals.filter(m=>m.id!==id);save();
  ...
  function bindProEvents() {
    ...
    document.getElementById("saveMealProBtn").onclick=async()=>{
      ...
      const meal={id:crypto.randomUUID?.()||String(Date.now()),date:iso(nutritionDate),type:document.getElementById("mealTypePro").value,time:document.getElementById("mealTimePro").value,description,normalizedName:pendingMealEstimate.normalizedName,calories:+document.getElementById("estimatedCaloriesPro").value||0,protein:+document.getElementById("estimatedProteinPro").value||0,carbs:+document.getElementById("estimatedCarbsPro").value||0,fat:+document.getElementById("estimatedFatPro").value||0,fiber:pendingMealEstimate.fiber,confidence:pendingMealEstimate.confidence,confidencePercent:pendingMealEstimate.confidencePercent,estimatedTotalWeight:pendingMealEstimate.estimatedTotalWeight,items:pendingMealEstimate.items,assumptions:pendingMealEstimate.assumptions,source:verifiedProductPro?"barcode_verified":(mealPhotoDataPro?"ai_visual_estimate":"ai_estimate"),verifiedProduct:verifiedProductPro,hasMealPhoto:Boolean(mealPhotoDataPro),consumedWholeProduct:consumeWholeProductPro};
      ...
      save();await saveMealRecordToCloud(meal);closeModal("mealComposerProModal");renderNutrition();toast("Comida registrada");
    };
