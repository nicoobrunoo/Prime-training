(()=>{
  ...
  document.addEventListener('click',event=>{
    ...
    set.done=!set.done;
    if(set.done&&!Number(set.reps))set.reps=Number(exercise.maxReps)||Number(exercise.minReps)||1;
  ...
  async function smartRecommendation(ex,session){
    ...
    try{
      ...
      // Regla de seguridad: la IA explica, pero no contradice la progresión objetiva.
      // Solo se baja si la lógica local detectó series por debajo del mínimo o una caída realmente marcada.
      // Solo se sube si todas las series alcanzaron el máximo con rendimiento estable.
      if(status==='reduce'&&local.status!=='reduce')status='maintain';
      if(status==='increase'&&local.status!=='increase')status='maintain';
      ...
      const contradicted=String(ai.status||'').toLowerCase()!==status;
      return{...local,status,proposed:roundHalf(proposed),reason:(!contradicted&&aiReason.length>35)?aiReason:local.reason,confidence:Number(ai.confidence_percent)||70};
  ...
  async function generateDailyReportPro(date=new Date().toISOString().slice(0,10)){
    ...
    let report;
    try{
      const physique=(data.physiqueCheckins||[]).find(x=>x.date===date)||null;const result=await callSmartHandler({action:'daily_expert_report',context:{date,profile,nutrition_settings:data.nutritionSettings,body,session,meals,physique_checkin:physique?{date:physique.date,weight:physique.weight,analysis:physique.analysis,has_photo:true}:null,physique_goal:data.physiqueGoal?.analysis||null,computed:{totals,target,meal_count:meals.length,main_meals:[...new Set(meals.map(m=>m.type||m.meal_type))],calorie_coverage:target.calories?totals.calories/target.calories:null,protein_coverage:target.protein?totals.protein/target.protein:null,load_changes:session?.loadChanges||[]},recent_sessions:(data.sessions||[]).filter(s=>s.date<date).slice(-8),recent_reports:(data.dailyReports||[]).slice(-5)}});
      report=result.report||result.analysis||result;
    }catch(error){report=deterministicReport(date,session,meals,error)}
    const physiqueForReport=(data.physiqueCheckins||[]).find(x=>x.date===date)||null;let physiquePhotoData=physiqueForReport?.photo||null;if(physiquePhotoData&&/^https?:/i.test(physiquePhotoData)){try{const blob=await fetch(physiquePhotoData).then(r=>{if(!r.ok)throw new Error('photo');return r.blob()});physiquePhotoData=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(String(fr.result));fr.onerror=reject;fr.readAsDataURL(blob)})}catch(_){physiquePhotoData=null}}report={id:crypto.randomUUID?.()||String(Date.now()),date,createdAt:new Date().toISOString(),physiquePhoto:physiquePhotoData,physiqueAnalysis:physiqueForReport?.analysis||null,...report};
  ...
  window.downloadPrimeReportPro=function(input){
    ...
    let y=59;if(report.physiquePhoto){try{doc.addImage(report.physiquePhoto,'JPEG',145,53,51,68,undefined,'FAST');doc.setTextColor(13,24,36);doc.setFontSize(17);doc.text(report.headline||'Cierre diario',14,y);y+=8;y=textBlock(doc,report.executive_summary||report.summary||'',14,y,124,10.5);doc.setTextColor(102,115,129);doc.setFontSize(7.5);doc.text('Check-in visual del día',170,124,{align:'center'});y=Math.max(y,132)}catch(_){doc.setTextColor(13,24,36);doc.setFontSize(17);doc.text(report.headline||'Cierre diario',14,y);y+=8;y=textBlock(doc,report.executive_summary||report.summary||'',14,y,182,10.5)}}else{doc.setTextColor(13,24,36);doc.setFontSize(17);doc.text(report.headline||'Cierre diario',14,y);y+=8;y=textBlock(doc,report.executive_summary||report.summary||'',14,y,182,10.5)};
