(()=>{
  ...
  .pai-avatar{width:34px;height:34px;flex:0 0 34px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(145deg,rgba(118,105,255,.28),rgba(38,201,234,.16));border:1px solid rgba(255,255,255,.1);color:#35d7ee;margin-right:10px}.pai-avatar svg{width:18px;height:18px}.pai-typing-row{align-items:center}.pai-delete svg,.pai-send svg{display:block!important;stroke:currentColor!important;fill:none!important;stroke-width:2!important}.pai-delete{box-shadow:inset 0 0 0 1px rgba(255,92,110,.08)}
  @media(max-width:760px){body.ai-chat-active .bottom-nav{position:fixed!important;left:50%!important;right:auto!important;top:auto!important;bottom:max(7px,env(safe-area-inset-bottom))!important;transform:translateX(-50%)!important;margin:0!important}.pai-main{height:100%!important}.pai-header{padding-left:16px!important;padding-right:16px!important}.pai-messages{padding-bottom:14px!important}.pai-footer{padding-bottom:7px!important}}
  `;
  ...
  function render(){
    ...
    const msgs=c?.messages||[];const waiting=Boolean(pendingChatId)&&pendingChatId===activeId;
    $("paiMessages").innerHTML=!msgs.length&&!waiting?`<div class="pai-welcome"><div><div class="pai-orb">✦</div><h2>Hola, ${esc(profile?.name||"Nicolás")}</h2><p>¿Qué necesitás hoy? Puedo ayudarte con tu entrenamiento, alimentación, progreso o planificación.</p></div></div>`:msgs.map(m=>`<div class="pai-msg ${m.role}"><div class="pai-bubble">${m.role==="assistant"?markdown(m.content):`<p>${esc(m.content)}</p>`}</div></div>`).join("")+(waiting?`<div class="pai-msg assistant pai-typing-row"><div class="pai-avatar"><svg><use href="#i-brain"/></svg></div><div class="pai-bubble"><div class="pai-thinking" aria-label="Prime AI está respondiendo"><i></i><i></i><i></i></div></div></div>`:"");
