...
- Solo sugerí reducir carga cuando al menos una serie efectiva quede por debajo del mínimo configurado, o cuando varias series queden por debajo y el patrón sea claro.
- Una diferencia pequeña dentro del rango, por ejemplo 20, 20, 20 y 19 con rango 15-20, NO justifica bajar: corresponde mantener.
- Si completa todas las series en el máximo del rango, con rendimiento estable, sugerí un aumento pequeño.
- Si todas las series están dentro del rango pero no todas alcanzan el máximo, mantené la carga.
...
- Los índices son métricas internas orientativas de 0 a 100 para seguir tendencias dentro de Prime Training.
- Si es el primer registro, usá una línea de base cercana a 50 y diferenciá grupos solo cuando sea visualmente razonable.
- Si hay análisis previo, evitá cambios mayores a 5 puntos por check-in salvo diferencia visual muy clara.
- Si la foto no permite evaluar un grupo, mantené su índice previo o usá 50 y aclará la limitación.
- Priorizá observaciones útiles y concretas, sin halagos vacíos ni críticas agresivas.
...
- Los target_indices son una métrica visual interna de Prime Training, no porcentajes anatómicos.
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (action === "analyze_physique_checkin") {
      const imageDataUrl = typeof body.imageDataUrl === "string" && body.imageDataUrl.startsWith("data:image/")
        ? body.imageDataUrl
        : undefined;
      if (!imageDataUrl) return jsonResponse({ ok: false, error: "Falta la foto del check-in." }, 400);
      if (imageDataUrl.length > 9_000_000) return jsonResponse({ ok: false, error: "La imagen es demasiado pesada." }, 413);
      const context = body.context && typeof body.context === "object"
        ? body.context as Record<string, unknown>
        : {};
      const analysis = await callGlobantAI(
        buildPhysiqueCheckinPrompt(context),
        crypto.randomUUID(),
        imageDataUrl,
      );
    ...
    if (action === "analyze_physique_goal") {
      const imageDataUrl = typeof body.imageDataUrl === "string" && body.imageDataUrl.startsWith("data:image/")
        ? body.imageDataUrl
        : undefined;
      if (!imageDataUrl) return jsonResponse({ ok: false, error: "Falta la foto objetivo." }, 400);
      if (imageDataUrl.length > 9_000_000) return jsonResponse({ ok: false, error: "La imagen es demasiado pesada." }, 413);
      const context = body.context && typeof body.context === "object"
        ? body.context as Record<string, unknown>
        : {};
      const analysis = await callGlobantAI(
        buildPhysiqueGoalPrompt(context),
        crypto.randomUUID(),
        imageDataUrl,
      );
      return jsonResponse({ ok: true, analysis });
